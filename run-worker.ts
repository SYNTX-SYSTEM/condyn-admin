  ...
  const worker = new CareerJobWorker(
    ...
    async (job) => {
      ...
      if (!validationResult.success) {
        throw new Error((validationResult as any).issues?.map((e: any) => e.message).join(", ") || "Validation failed");
      }
      ...
      const { PostgresCareerAnalysisRepository } = require("./lib/career/repositories/postgres");

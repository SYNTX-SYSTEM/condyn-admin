...
The declarer is declared input only and may differ from the decision actor. `ACTION INTENT DECLARER != DECISION ACTOR != FUTURE COMMITMENT ACTOR != FUTURE ACTION ACTOR`; `HUMAN_INPUT != AUTHENTICATED IDENTITY != AUTHORIZATION != SIGNATURE != PERMISSION != TRUTH`. The opaque operation description is not an executable command, target, assignee, workflow, schedule, authorization, execution proof, or expected outcome.

...
## Phase 8D8 decision-context-observation-context-validation-assembly boundary

Phase 8D8 is a deterministic derivational-coherence operation only: `DecisionContextObservationContextTransition + explicit DecisionContextValidationAssemblyInput -> assembleDecisionContextValidation against exactly transition.context -> DecisionContextObservationContextValidationAssembly -> STOP`. It retains the complete sealed transition, complete explicit input, and complete resulting assembly without establishing validation completeness, truth, authority, or revision state.

The embedded `DecisionContextDraft` remains `validationStatus: NOT_RUN` before and after Phase 8D8. VALIDATION ASSEMBLY EXISTENCE != CONTEXT VALIDATION STATUS. ASSEMBLY SUCCESS != VALIDATION_STATUS CHANGE. ASSEMBLY SUCCESS != VALIDATED CONTEXT.

BASE REVISION VALIDATION != NEW CONTEXT VALIDATION. BASE VALIDATION INPUT != AUTOMATIC NEW VALIDATION INPUT. BASE VALIDATION ASSEMBLY != NEW CONTEXT VALIDATION ASSEMBLY. VALIDATION INPUT REUSE != VALIDATION STATE CARRY-FORWARD. OLD DERIVATION != NEW CONTEXT DERIVATION. Validation input is explicitly supplied, never copied from the bound base revision. The existing assembly contract reconstructs `StructuralExpectation`, `StructuralGap`, `StructuralConsequence`, assembly identity, and canonicalization against the new Context; Phase 8D8 duplicates none of those algorithms. Existing precise derivation errors may propagate from creation.

VALIDATION INPUT != VALIDATION COMPLETENESS. VALIDATION INPUT != TRUTH. VALIDATION INPUT != AUTHORITY. VALIDATION INPUT != HUMAN DECISION. VALIDATION INPUT != REVISION. VALIDATION INPUT != PERSISTENCE. EMPTY VALIDATION INPUT != COMPLETE VALIDATION. NONEMPTY VALIDATION INPUT != COMPLETE VALIDATION. ASSEMBLY SUCCESS != COMPLETE VALIDATION. A formerly coherent base derivation may be incoherent when its old input is explicitly reassembled against the changed Context; it is not rewritten to pass.

The exact six fields are `artifactKind`, `schemaVersion`, `decisionContextObservationContextValidationAssemblyId`, `decisionContextObservationContextTransition`, `validationInput`, and `validationAssembly`. `DCOCVA_` is SHA-256 over `[schema, canonical complete transition, canonical complete validation input, canonical complete validation assembly]`, first 24 uppercase hex. Object insertion order is non-semantic. The identity is not truth, authority, completeness, or persistence proof. Stored assertion is self-contained: it reasserts coherence through `assertDecisionContextValidationAssembly(transition.context, stored validationInput, stored validationAssembly)`, does not trust `assemblyId` alone, repairs nothing, and gives body invalidity precedence over stale DCOCVA ID.
...
## Phase 8D10 revision persistence boundary

`DecisionContextObservationRevisionCreation -> BoundDecisionContextRevisionPersister.persist -> exact returned DecisionContextRevision -> DecisionContextObservationRevisionPersistence -> STOP`. The wrapper submits only the complete sealed 8D9 child revision, accepts only complete-data equality on return, and retains complete predecessor/result state. `DCORP_` commits to canonical complete creation plus persisted revision; it is not a fresh repository query.

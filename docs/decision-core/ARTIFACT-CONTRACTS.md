...
## Phase 8E2 decision-loop occurrence-return binding

`DecisionLoopOccurrenceReturnBinding` is `DECISION_LOOP_OCCURRENCE_RETURN_BINDING_V1` / `DECISION_LOOP_OCCURRENCE_RETURN_BINDING`. It contains exactly `artifactKind`, `schemaVersion`, `decisionLoopOccurrenceReturnBindingId`, `humanCommitmentActionOccurrenceAssociationProposal`, and `decisionContextObservationRevisionPersistence`. It retains the complete sealed 8E1 decision-side bridge and complete sealed 8D10 persisted return-path lineage. It contains no duplicate ActionOccurrenceClaim and no `loopClosed`, closure-status, success, execution-status, fulfillment, causation, feedback, learning, truth, authority-certificate, current/head/latest, or timestamp field.

The bridge claim is extracted from the retained `humanCommitmentActionOccurrenceAssociationProposal.actionOccurrenceClaim`. The return claim is extracted only through the complete retained 8D10 lineage. They must be complete-data equal in `artifactKind`, `schemaVersion`, `actionOccurrenceClaimId`, `source`, and `operationDescription`; action-occurrence-claim ID equality alone is insufficient. No independent claim, claim ID, repository lookup, Context search, similarity, text/actor/source/temporal matching, or model inference is admitted. A valid bridge and a valid return path with different complete claims are not a valid binding.

`DLORB_` is SHA-256, first 24 uppercase hexadecimal characters, over `[DECISION_LOOP_OCCURRENCE_RETURN_BINDING_V1, canonical complete HumanCommitmentActionOccurrenceAssociationProposal, canonical complete DecisionContextObservationRevisionPersistence]`. Object insertion order is non-semantic; existing sealed order-bearing arrays are not sorted. DLORB IDENTITY != EXECUTION PROOF. DLORB IDENTITY != RELATION TRUTH. DLORB IDENTITY != CAUSAL PROOF. DLORB IDENTITY != FRESH REPOSITORY PROOF.

Creation and stored assertion are descriptor-safe and self-contained. Stored assertion asserts both complete parents, extracts and compares the retained complete claims, then verifies DLORB identity. It performs no repository operation, persistence, parent lookup, authority resolution, current/head/latest selection, model/provider operation, or external lineage reconstruction; it repairs nothing. Cross-lineage body invalidity precedes a stale outer DLORB ID. The exact owned errors are `ERR_DECISION_LOOP_OCCURRENCE_RETURN_BINDING_INPUT_INVALID`, `ERR_DECISION_LOOP_OCCURRENCE_RETURN_BINDING_ASSOCIATION_INVALID`, `ERR_DECISION_LOOP_OCCURRENCE_RETURN_BINDING_PERSISTENCE_INVALID`, `ERR_DECISION_LOOP_OCCURRENCE_RETURN_BINDING_OCCURRENCE_MISMATCH`, `ERR_DECISION_LOOP_OCCURRENCE_RETURN_BINDING_INVALID`, and `ERR_DECISION_LOOP_OCCURRENCE_RETURN_BINDING_ID_MISMATCH`.

One valid artifact establishes REPRESENTED STRUCTURAL LOOP CLOSURE for its exact sealed lineages, not execution proof, action fact, commitment fulfillment, relation truth, causation, outcome or observation truth, semantic correctness, decision success, feedback, learning, current repository proof, current record existence, current authority, or current/head/latest revision. No separate loop-closure certificate or Phase 8E3 is required for structural closure in the current architecture.
...
Construction takes exactly the sealed transition and caller-supplied explicit input. It uses `assembleDecisionContextValidation(transition.context, capturedValidationInput)` and leaves the retained `transition.context.validationStatus` as `NOT_RUN`. The existing Validation Assembly contract owns Structural Expectation, Structural Gap, Structural Consequence, assembly identity, and canonicalization. Existing precise assembly or derivation errors may propagate unchanged from creation.

`DCOCVA_` is the first 24 uppercase hexadecimal characters of SHA-256 over `[schema, canonical complete DecisionContextObservationContextTransition, canonical complete DecisionContextValidationAssemblyInput, canonical complete DecisionContextValidationAssembly]`. Object insertion order is non-semantic. This identity is not truth, authority, completeness, or persistence proof.

Stored assertion is self-contained: it captures transition, input, and assembly safely, then calls `assertDecisionContextValidationAssembly` against exactly `transition.context`, stored input, and stored assembly. It performs no reader call, authority resolution, repository read, Context construction, revision construction, or persistence operation; it does not merely trust `assemblyId`; body invalidity precedes DCOCVA mismatch; and it repairs nothing. Descriptor-safe capture rejects accessors without getter execution and rejects symbols, hidden and extra properties, and hostile nested derivation state where applicable. Returned data is detached; no deep freeze is claimed.

VALIDATION ASSEMBLY EXISTENCE != CONTEXT VALIDATION STATUS. ASSEMBLY SUCCESS != VALIDATION_STATUS CHANGE. ASSEMBLY SUCCESS != VALIDATED CONTEXT. VALIDATION ASSEMBLY != TRUTH. VALIDATION ASSEMBLY != COMPLETENESS. VALIDATION ASSEMBLY != CURRENT AUTHORITY. VALIDATION ASSEMBLY != SEMANTIC VERIFICATION. VALIDATION ASSEMBLY != DECISION READINESS. VALIDATION ASSEMBLY != REVISION. VALIDATION ASSEMBLY != REVISION CREATION. VALIDATION ASSEMBLY != REVISION TRANSITION. VALIDATION ASSEMBLY != PERSISTENCE. VALIDATION ASSEMBLY != PERSISTENCE AUTHORITY. VALIDATION ASSEMBLY != LOOP CLOSED.

The exact Phase 8D8-owned error surface is `ERR_DECISION_CONTEXT_OBSERVATION_CONTEXT_VALIDATION_ASSEMBLY_INPUT_INVALID`, `ERR_DECISION_CONTEXT_OBSERVATION_CONTEXT_VALIDATION_ASSEMBLY_TRANSITION_INVALID`, `ERR_DECISION_CONTEXT_OBSERVATION_CONTEXT_VALIDATION_ASSEMBLY_INVALID`, and `ERR_DECISION_CONTEXT_OBSERVATION_CONTEXT_VALIDATION_ASSEMBLY_ID_MISMATCH`.
...
Construction accepts exactly one sealed binding field. It derives the candidate `DCI_` identity only from retained projected `OBSERVATION` role, statement, and provenance using existing Context identity semantics. For `AUTHORITATIVE_STATE`, the exact projected reference must be in the bound base Context inventory or construction fails with `ERR_DECISION_CONTEXT_OBSERVATION_MATERIALIZATION_READINESS_SOURCE_REFERENCE_MISSING`. For every provenance, the candidate must be absent from the bound base Context item inventory or construction fails with `ERR_DECISION_CONTEXT_OBSERVATION_MATERIALIZATION_READINESS_ITEM_ALREADY_PRESENT`. `HUMAN_INPUT` and `MODEL_PROPOSAL` add no source-inventory requirement.

Stored assertion is self-contained: it performs no reader call, authority resolution, Context construction, revision construction, or persistence operation; rechecks exact representation, sealed binding, candidate identity, both applicable readiness conditions, and deterministic identity; repairs nothing; and treats malformed body invalidity before DCOMR mismatch. Descriptor-safe capture rejects accessors without getter execution and rejects hidden, symbol, and extra properties where applicable. The exact error surface is `ERR_DECISION_CONTEXT_OBSERVATION_MATERIALIZATION_READINESS_INPUT_INVALID`, `ERR_DECISION_CONTEXT_OBSERVATION_MATERIALIZATION_READINESS_BINDING_INVALID`, `ERR_DECISION_CONTEXT_OBSERVATION_MATERIALIZATION_READINESS_SOURCE_REFERENCE_MISSING`, `ERR_DECISION_CONTEXT_OBSERVATION_MATERIALIZATION_READINESS_ITEM_ALREADY_PRESENT`, `ERR_DECISION_CONTEXT_OBSERVATION_MATERIALIZATION_READINESS_INVALID`, and `ERR_DECISION_CONTEXT_OBSERVATION_MATERIALIZATION_READINESS_ID_MISMATCH`.

This is positive structural readiness only: no status, score, confidence, ranking, priority, recommendation, threshold, or human approval field exists. READINESS != MATERIALIZATION. READINESS != MATERIALIZATION AUTHORITY. READINESS != DECISION CONTEXT ITEM. READINESS != ITEM MEMBERSHIP. READINESS != CONTEXT MEMBERSHIP. READINESS != CONTEXT MUTATION. READINESS != REVISION MUTATION. READINESS != REVISION CREATION. READINESS != REVISION TRANSITION. READINESS != PERSISTENCE. READINESS != PERSISTENCE AUTHORITY. No item, Context, revision, source payload, authority result, truth flag, or timestamp is represented or created.

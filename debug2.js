...
async function test() {
  ...
  fs.writeFileSync('output2.txt', JSON.stringify({ success: result.success, issues: result.issues }, null, 2));
}
test().catch(err => fs.writeFileSync('output2.txt', err.toString()));

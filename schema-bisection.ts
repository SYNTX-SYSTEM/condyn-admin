...
import {
  ...
  CanonicalIdSchema
} from "./lib/career/schema-projector";
...
function walkSchema(node: any, path: string, depth: number) {
  if (!node || typeof node !== "object") return;
  maxDepth = Math.max(maxDepth, depth);

  if (Array.isArray(node)) {
    node.forEach((item, idx) => walkSchema(item, `${path}[${idx}]`, depth + 1));
    return;
  }
  ...
  if (keys.includes("$ref")) {
    ...
    if (nonRefKeys.length > 0) {
      illegalRefSiblings.set(path, nonRefKeys);
    }
  }
  ...
  for (const k of keys) {
    if (k === "properties") {
      ...
      for (const pk of Object.keys(props)) {
        propCount++;
        walkSchema(props[pk], `${path}.properties.${pk}`, depth + 1);
      }
      uniqueKeywords.add(k);
    } else if (k === "$defs") {
      const defs = node[k];
      for (const dk of Object.keys(defs)) {
        walkSchema(defs[dk], `${path}.$defs.${dk}`, depth + 1);
      }
    } else {
      uniqueKeywords.add(k);
      if (!SUPPORTED_KEYWORDS.has(k)) {
        unsupportedKeywords.set(path, k);
      }
      if (typeof node[k] === "object") {
        walkSchema(node[k], `${path}.${k}`, depth + 1);
      }
    }
  }
}
...
async function runPhase2() {
  ...
  // Keep defs around for any branch
  ...
  // We must import the schemas from schema.ts
  const { UniversalEntitySchema, OrganizationEntitySchema, RoleEntitySchema, SearchQueryEntitySchema, ConsistencyClusterSchema } = require("./lib/career/schema");

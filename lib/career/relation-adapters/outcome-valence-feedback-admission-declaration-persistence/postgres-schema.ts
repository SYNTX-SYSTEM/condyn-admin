...
import { careerOutcomeValenceDeclarations } from "../outcome-valence-declaration-persistence/postgres-schema";

export const careerOutcomeValenceFeedbackAdmissionDeclarations = pgTable(
  "career_outcome_valence_feedback_admission_declarations",
  {
    careerOutcomeValenceFeedbackAdmissionDeclarationId: text("career_outcome_valence_feedback_admission_declaration_id").primaryKey(),
    careerOutcomeValenceDeclarationId: text("career_outcome_valence_declaration_id").notNull()
      .references(
        () => careerOutcomeValenceDeclarations.careerOutcomeValenceDeclarationId,
    ...
    outcomeValenceDeclaredByActorId: text("outcome_valence_declared_by_actor_id").notNull(),
    ...
    outcomeValenceDeclaredAt: text("outcome_valence_declared_at").notNull(),
    valence: text("valence").notNull(),
    admittedByActorId: text("admitted_by_actor_id").notNull(),
    admittedAt: text("admitted_at").notNull(),
    admissionState: text("admission_state").notNull(),
    schemaVersion: text("schema_version").notNull(),
    createdAt: text("created_at").notNull(),
    payload: jsonb("payload").$type<CareerOutcomeValenceFeedbackAdmissionDeclaration>().notNull(),
  },
);

export const careerOutcomeValenceFeedbackAdmissionDeclarationSubjects = pgTable(
  "career_outcome_valence_feedback_admission_declaration_subjects",
  {
    careerOutcomeValenceFeedbackAdmissionDeclarationId: text("career_outcome_valence_feedback_admission_declaration_id").notNull()
      .references(
  ...
  table => [primaryKey({ columns: [
    table.careerOutcomeValenceFeedbackAdmissionDeclarationId,
    table.recommendationProposalId,
    table.sourceEvolutionInputItemOrdinal,
  ] })],
);

export const careerOutcomeValenceFeedbackAdmissionDeclarationEvidenceReferences = pgTable(

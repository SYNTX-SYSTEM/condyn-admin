...
export const PrimitiveVisuals = {
  ...
  driftToTremble(globalDrift: number): { amount: number; speed: number; active: boolean } {
    return { amount: globalDrift * 1.5, speed: 80 + (globalDrift * 40), active: globalDrift > 0.7 };
  },
  ...
  delayToFlowDuration(delay: number): number { return 6 + (delay * 4); },
  shouldAnimateFlow(tier: VisualTier): boolean { return tier !== 'background'; },
  shouldShowLabel(_tier: VisualTier, _isHovered: boolean): boolean { return true; },
  labelFontSize(tier: VisualTier): number { return tier === 'primary' ? 13 : 11; },

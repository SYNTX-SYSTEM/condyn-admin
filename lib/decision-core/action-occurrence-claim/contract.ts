...
import {
  ACTION_OCCURRENCE_CLAIM_SCHEMA_VERSION,
  type ActionOccurrenceClaim,
  type ActionOccurrenceClaimInput,
  type ActionOccurrenceClaimSource
} from "./types";

const fail = (code: string): never => { throw new Error(code); };
const inputKeys = ["source", "operationDescription"] as const;
const claimKeys = ["artifactKind", "schemaVersion", "actionOccurrenceClaimId", "source", "operationDescription"] as const;
...
const idPattern = /^DAOC_[0-9A-F]{24}$/;
...
function source(value: unknown, trimHumanActor: boolean, sourceCode: string, referenceCode: string): ActionOccurrenceClaimSource {
...
function operation(value: unknown, trim: boolean, code: string): string {
...
function canonicalSource(source: ActionOccurrenceClaimSource): readonly ["HUMAN_INPUT", string] | readonly ["AUTHORITATIVE_STATE", readonly [string, string, string, string]] {
...
function claimId(source: ActionOccurrenceClaimSource, operationDescription: string): string {
...
function construct(source: ActionOccurrenceClaimSource, operationDescription: string): ActionOccurrenceClaim {
  return {
    artifactKind: "ACTION_OCCURRENCE_CLAIM",
    schemaVersion: ACTION_OCCURRENCE_CLAIM_SCHEMA_VERSION,
    actionOccurrenceClaimId: claimId(source, operationDescription),
    source,
    operationDescription
  };
}

export function createActionOccurrenceClaim(input: ActionOccurrenceClaimInput): ActionOccurrenceClaim {
  const captured = exact(input, inputKeys, "ERR_DECISION_ACTION_OCCURRENCE_CLAIM_INPUT_INVALID");
  const claimSource = source(captured.source, true, "ERR_DECISION_ACTION_OCCURRENCE_CLAIM_SOURCE_INVALID", "ERR_DECISION_ACTION_OCCURRENCE_CLAIM_REFERENCE_INVALID");
  const operationDescription = operation(captured.operationDescription, true, "ERR_DECISION_ACTION_OCCURRENCE_CLAIM_OPERATION_INVALID");
  const claim = construct(claimSource, operationDescription);
  assertActionOccurrenceClaim(claim);
  return structuredClone(claim);
}

export function assertActionOccurrenceClaim(value: unknown): asserts value is ActionOccurrenceClaim {
  const invalid = "ERR_DECISION_ACTION_OCCURRENCE_CLAIM_INVALID";
  try {
    const claim = exact(value, claimKeys, invalid);
    if (claim.artifactKind !== "ACTION_OCCURRENCE_CLAIM" || claim.schemaVersion !== ACTION_OCCURRENCE_CLAIM_SCHEMA_VERSION || typeof claim.actionOccurrenceClaimId !== "string" || !idPattern.test(claim.actionOccurrenceClaimId)) fail(invalid);
    const claimSource = source(claim.source, false, invalid, invalid);
    const operationDescription = operation(claim.operationDescription, false, invalid);
    if (claim.actionOccurrenceClaimId !== claimId(claimSource, operationDescription)) fail("ERR_DECISION_ACTION_OCCURRENCE_CLAIM_ID_MISMATCH");
  } catch (error) {
    if (error instanceof Error && error.message === "ERR_DECISION_ACTION_OCCURRENCE_CLAIM_ID_MISMATCH") throw error;
    return fail(invalid);
  }
}

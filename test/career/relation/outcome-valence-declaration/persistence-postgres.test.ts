...
const loadAdapter = () =>
  import("../../../../lib/career/relation-adapters/outcome-valence-declaration-persistence") as Promise<any>;
const loadSchema = () =>
  import("../../../../lib/career/relation-adapters/outcome-valence-declaration-persistence/postgres-schema") as Promise<any>;
...
async function ready() {
  ...
  await createTables(base.first.db, [
    ...
    schema.careerOutcomeValenceDeclarations, schema.careerOutcomeValenceDeclarationSubjects, schema.careerOutcomeValenceDeclarationEvidenceReferences,
  ]);
  const value = createT12IHistoricalFixture();
  ...
  const outcomeValences = new adapter.PostgresCareerOutcomeValenceDeclarationRepository(base.first.db, outcomeRoles);
  return { ...base, value, adapter, schema, outcomeRoles, outcomeValences };
}
...
async function scoped(database: any, base: Awaited<ReturnType<typeof ready>>) {
  ...
  return { outcomeValences: new base.adapter.PostgresCareerOutcomeValenceDeclarationRepository(database, outcomeRoles) };
}
...
function declaration(base: Awaited<ReturnType<typeof ready>>) {
  ...
  return createCareerOutcomeValenceDeclaration(base.value.outcomeRoleDeclaration, {
    careerOutcomeRoleDeclarationId: base.value.outcomeRoleDeclaration.careerOutcomeRoleDeclarationId,
    declaredByActorId: `OUTCOME_VALENCE_DECLARANT_T12I_${suffix}`,
    declaredAt: "2027-02-07T01:00:00.000Z",
    valence: "DESIRABLE",
    valenceEvidenceRefs: [`evidence://outcome-valence/t12i/${suffix}/b`, `evidence://outcome-valence/t12i/${suffix}/a`],
    createdAt: "2027-02-07T01:00:01.000Z",
  });
}

describe("T12I PostgreSQL immutable CareerOutcomeValenceDeclaration RED contract", () => {
  it("constructs canonical RCP -> DAR -> DCTXREV -> DCR -> DAINT -> HCOM -> EAGR -> ECTXREV -> AOC -> SCD -> ASCAD -> CORD -> COVD history before the adapter boundary", () => {
    const value = createT12IHistoricalFixture();
    const valence = createCareerOutcomeValenceDeclaration(value.outcomeRoleDeclaration, value.outcomeValenceDeclarationInput);
    expect(valence.careerOutcomeRoleDeclarationId).toBe(value.outcomeRoleDeclaration.careerOutcomeRoleDeclarationId);
    expect(valence.outcomeRoleDeclaredByActorId).toBe(value.outcomeRoleDeclaration.declaredByActorId);
    expect(valence.declaredByActorId).not.toBe(value.occurrence.performedByActorId);
    expect(valence.declaredByActorId).not.toBe(value.stateChangeDeclaration.observedByActorId);
    expect(valence.declaredByActorId).not.toBe(value.associationDeclaration.declaredByActorId);
    expect(valence.declaredByActorId).not.toBe(value.outcomeRoleDeclaration.declaredByActorId);
    ...
    expect(valence.declaredAt > value.grant.effectiveUntil!).toBe(true);
  });

  it("freezes only the repository API, exact root columns, normalized children, and direct restrictive CORD predecessor", async () => {
    const adapter = await loadAdapter();
    const schema = await loadSchema();
    expect(adapter.PostgresCareerOutcomeValenceDeclarationRepository).toBeTypeOf("function");
    const root = getTableConfig(schema.careerOutcomeValenceDeclarations);
    expect(root.name).toBe("career_outcome_valence_declarations");
    expect(root.columns.map(column => column.name)).toEqual([
      "career_outcome_valence_declaration_id", "career_outcome_role_declaration_id", "career_action_state_change_association_declaration_id", "career_state_change_declaration_id", "career_action_occurrence_id", "career_execution_context_revision_id", "career_execution_authority_grant_revision_id", "career_human_commitment_id", "career_decision_action_intent_id", "human_decision_record_id", "career_decision_context_revision_id", "decision_authority_grant_revision_id", "recommendation_proposal_id", "performed_by_actor_id", "observed_by_actor_id", "association_declared_by_actor_id", "outcome_role_declared_by_actor_id", "source_declaration_class", "source_action_intent_class", "operation_description", "execution_authority_scope", "execution_target_kind", "execution_target_ref", "execution_channel_kind", "execution_channel_ref", "action_occurred_at", "state_subject_kind", "state_subject_ref", "state_dimension", "before_observation_state", "before_observation_value", "after_observation_state", "after_observation_value", "observed_at", "association_declared_at", "outcome_role_declared_at", "declared_by_actor_id", "declared_at", "valence", "schema_version", "created_at", "payload",
    ]);
    expect(getTableConfig(schema.careerOutcomeValenceDeclarationSubjects).name).toBe("career_outcome_valence_declaration_subjects");
    expect(getTableConfig(schema.careerOutcomeValenceDeclarationEvidenceReferences).name).toBe("career_outcome_valence_declaration_evidence_references");
    expect(root.foreignKeys).toHaveLength(1);
    const predecessor = root.foreignKeys[0].reference();
    expect(predecessor.columns.map(column => column.name)).toEqual(["career_outcome_role_declaration_id"]);
    expect(getTableConfig(predecessor.foreignTable).name).toBe("career_outcome_role_declarations");
    expect(predecessor.foreignColumns.map(column => column.name)).toEqual(["career_outcome_role_declaration_id"]);
    expect(root.foreignKeys[0].onDelete).toBe("restrict");
    expect(root.foreignKeys[0].onUpdate).toBe("restrict");
  });

  it("persists exact root/payload/children/CORD history, performs detached reread, and preserves genuine absence", async () => {
    const base = await ready();
    const value = declaration(base);
    await expect(base.outcomeValences.persistCareerOutcomeValenceDeclaration(value)).resolves.toEqual(value);
    const first = await base.outcomeValences.getCareerOutcomeValenceDeclarationById(value.careerOutcomeValenceDeclarationId);
    expect(first).toEqual(value);
    first.stateSubject.subjectRef = "local";
    await expect(base.outcomeValences.getCareerOutcomeValenceDeclarationById(value.careerOutcomeValenceDeclarationId)).resolves.toEqual(value);
    await expect(base.outcomeValences.getCareerOutcomeValenceDeclarationById(missing))
      .rejects.toThrow("ERR_CAREER_OUTCOME_VALENCE_DECLARATION_NOT_FOUND");
  });

  it("is idempotent only for exact complete durable state including audit time, valence, copied witnesses, and children", async () => {
    const base = await ready();
    const value = declaration(base);
    await base.outcomeValences.persistCareerOutcomeValenceDeclaration(value);
    await expect(base.outcomeValences.persistCareerOutcomeValenceDeclaration(value)).resolves.toEqual(value);
    for (const changed of [
      { ...value, createdAt: "2030-01-01T00:00:00.000Z" }, { ...value, valence: "UNDESIRABLE" },
      { ...value, declaredByActorId: "OTHER" }, { ...value, declaredAt: "2027-02-07T02:00:00.000Z" },
      { ...value, outcomeRoleDeclaredAt: "2027-02-06T02:00:00.000Z" }, { ...value, operationDescription: "other" },
      { ...value, decisionSubjects: value.decisionSubjects.slice(0, 1) }, { ...value, valenceEvidenceRefs: ["evidence://other"] },
    ...
    ]) await expect(base.outcomeValences.persistCareerOutcomeValenceDeclaration(changed)).rejects
      .toThrow("ERR_CAREER_OUTCOME_VALENCE_DECLARATION_IMMUTABLE_CONFLICT");
  });

  it("fails closed for physical/payload/child/identity/CORD corruption without translating found roots to absence", async () => {
    const base = await ready();
    const value = declaration(base);
    const id = value.careerOutcomeValenceDeclarationId;
    await base.outcomeValences.persistCareerOutcomeValenceDeclaration(value);
    await rollback(base, async (transaction, graph) => {
      await transaction.execute(drizzleSql.raw(`UPDATE career_outcome_valence_declarations SET declared_by_actor_id='BROKEN' WHERE career_outcome_valence_declaration_id='${id}'`));
      await expect(graph.outcomeValences.getCareerOutcomeValenceDeclarationById(id))
        .rejects.toThrow("ERR_CAREER_OUTCOME_VALENCE_DECLARATION_PERSISTENCE_FAILED");
    });
  });

  it("requires exact CORD copied witnesses and COVD chronology but never re-evaluates EAGR or DAR after AOC time", async () => {
    const base = await ready();
    const value = declaration(base);
    expect(value.declaredAt >= base.value.outcomeRoleDeclaration.declaredAt).toBe(true);
    ...
    for (const changed of [
      ...
      { ...value, outcomeRoleDeclaredByActorId: "OTHER" }, { ...value, associationDeclaredByActorId: "OTHER" },
      { ...value, performedByActorId: "OTHER" }, { ...value, observedByActorId: "OTHER" },
      { ...value, actionOccurredAt: "2027-02-03T01:00:01.000Z" }, { ...value, declaredAt: "2027-02-06T00:59:59.999Z" },
    ]) await expect(base.outcomeValences.persistCareerOutcomeValenceDeclaration(changed)).rejects.toThrow();
  });

  it("atomically rolls back deterministic root, subject, and evidence write failures before exact retry", async () => {
    const base = await ready();
    const tables = ["career_outcome_valence_declarations", "career_outcome_valence_declaration_subjects", "career_outcome_valence_declaration_evidence_references"] as const;
    for (const table of tables) {
      const value = declaration(base);
      const suffix = String(++unique);
      const fn = `fail_covd_${suffix}`;
      const trigger = `trigger_covd_${suffix}`;
      ...
      await expect(base.outcomeValences.persistCareerOutcomeValenceDeclaration(value))
        .rejects.toThrow("ERR_CAREER_OUTCOME_VALENCE_DECLARATION_PERSISTENCE_FAILED");
      for (const durable of tables) {
        const rows = await base.first.sql.unsafe(`SELECT count(*)::int AS count FROM ${durable} WHERE career_outcome_valence_declaration_id='${value.careerOutcomeValenceDeclarationId}'`);
  ...
  it("requires mandatory post-commit exact reread and exposes no mutable, current, verification, evaluation, or causal surface", async () => {
    const base = await ready();
    const surface = Object.getOwnPropertyNames(Object.getPrototypeOf(base.outcomeValences));
    expect(surface).toEqual(expect.arrayContaining([
      "getCareerOutcomeValenceDeclarationById", "persistCareerOutcomeValenceDeclaration",
    ]));
    expect(surface).not.toEqual(expect.arrayContaining([
      ...
      "getHeadCareerOutcomeValenceDeclaration", "updateCareerOutcomeValenceDeclaration",
      "repairCareerOutcomeValenceDeclaration", "replaceCareerOutcomeValenceDeclaration",
      "supersedeCareerOutcomeValenceDeclaration", "regenerateCareerOutcomeValenceDeclaration",
      "verifyCareerOutcomeValence", "evaluateCareerOutcomeValence", "satisfyCareerOutcomeValence",
      "attributeCareerOutcomeValence", "createFeedback", "createLearning",
    ]));
  });
});

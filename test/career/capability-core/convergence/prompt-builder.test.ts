...
describe("Capability Convergence prompt builder", () => {
  it("uses the secret only as system prompt and deterministically transports eligible verified evidence", () => {
    ...
    const [first, second] = [a.candidateId, b.candidateId].sort((left, right) => left < right ? -1 : left > right ? 1 : 0); expect(prompt.systemPrompt).toBe(`${secret}\n\nOutput identity contract:\n"convergence_version" MUST equal exactly "v".\nDo not substitute the prompt slug, schema version, model version, semantic version shorthand, or any other value.`); expect(prompt.userPrompt).not.toContain(secret); expect(prompt.userPrompt.indexOf(first)).toBeLessThan(prompt.userPrompt.indexOf(second)); expect(prompt.userPrompt.indexOf("EVD_A")).toBeLessThan(prompt.userPrompt.indexOf("EVD_Z"));
  });
  ...
  it("binds the provider-visible output identity to the resolved Convergence kernel version", () => {
    const prompt = buildCapabilityConvergencePrompt([], {
      kernelVersion: "convergence-v1",
      templateId: "T",
      versionId: "V",
      checksum: "C",
      plainTextContent: "CONVERGENCE_KERNEL"
    });

...
import type { AuthenticatedCapabilityVerificationChain, CapabilityVerificationIntegrityInput, CapabilityVerificationRun } from "../../../../lib/career/capability-core/verification";
...
describe("Phase 4 Slice 2 verification-run integrity contract", () => {
  ...
  it.each([
    ...
    ["invalid verified-level shape", (input: CapabilityVerificationIntegrityInput) => ({ ...input, verificationRun: { ...input.verificationRun, payload: { ...input.verificationRun.payload, demonstratedLevelOutcomes: [{ ...input.verificationRun.payload.demonstratedLevelOutcomes[0], status: "VERIFIED", demonstratedCapabilityLevel: null }] } } })]
  ])("rejects verification payload coverage violation: %s", async (_name, mutate) => {
    await expect(authenticate(mutate(authenticatedFixture()))).rejects.toThrow("ERR_CAPABILITY_VERIFICATION_RUN_INTEGRITY_INVALID");
  });
  ...
  it("freezes the next publisher input to authenticated artifacts only", () => {
    expect(typeof (verification as Record<string, unknown>).publishAuthenticatedVerifiedCapabilitySnapshot).toBe("function");
  });

  it.each([
    ["unpersisted VFY", (_input: CapabilityVerificationIntegrityInput) => null],
    ["createdAt-only divergence", (input: CapabilityVerificationIntegrityInput) => ({ ...input.verificationRun, createdAt: "changed" })],
    ["completedAt-only divergence", (input: CapabilityVerificationIntegrityInput) => ({ ...input.verificationRun, completedAt: "changed" })],
    ["payload divergence", (input: CapabilityVerificationIntegrityInput) => ({ ...input.verificationRun, payload: { ...input.verificationRun.payload, publicationEligibility: "BLOCKED" as const } })]
  ])("rejects non-authoritative supplied VFY: %s", async (_name, persisted) => {
    const input = authenticatedFixture();
  ...
  it("permits only the exact immutable persisted VFY artifact", async () => {
    const input = authenticatedFixture();
    await expect(authenticatePersisted(input, structuredClone(input.verificationRun))).resolves.toBeDefined();
  });
});

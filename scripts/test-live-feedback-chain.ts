...
/**
 * Live execution script for TEST003D Acceptance Gate
 * Runs the full feedback attribution chain proof.
 */
async function main() {
  console.log("==================================================");
  console.log("TEST003D LIVE FEEDBACK CHAIN VERIFICATION");
  console.log("==================================================");
...
Worked at TechCorp (a US based SOFTWARE company) for 5 years leading a team of 4 engineers.
  ...
  console.log("\\n7. Outcome arrives later...");
  const t4 = new Date(new Date(t3).getTime() + 86400000 * 2).toISOString();
  const outcome = createOutcome(
      action,
      "System_ATS_Sync",
      "INTERVIEW_INVITATION_RECEIVED",
      t4,
      "Automated status change in Greenhouse."
  );

  console.log("\\n8. Authorized Actor provides Feedback and Attribution...");
  ...
  const feedback = createFeedback(
      ...
      "Human_Hiring_Manager_123",
      t5,
      "The profile was indeed strong enough for an interview."
  );
  ...
  const attribution = createAttribution(
      ...
      "SUPPORTS",
      "Human_Hiring_Manager_123",
      "The recommendation accurately identified the Distributed Systems strength."
  );

  console.log("==================================================");
  console.log("FEEDBACK TRACE");
  console.log(`[ATTRIBUTION] ${attribution.attributionType} ${attribution.targetType} | Actor: ${attribution.actor}`);
  console.log(`     ↓ INFORMS`);
  console.log(`[FEEDBACK]    ${feedback.evaluation} | Actor: ${feedback.actor} | at ${feedback.observedAt}`);
  console.log(`     ↓ OBSERVES`);
  console.log(`[OUTCOME]     ${outcome.outcomeState} | Actor: ${outcome.actor} | at ${outcome.occurredAt}`);
  ...
  if (supported && supported.capabilityProof) {
    ...
    console.log("\\n[SUCCESS] Full Feedback Attribution to Source Proof Chain Closed.");
  } else {
